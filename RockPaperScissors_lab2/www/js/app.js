...
lab2.factory('processMove', function() {
  return function() {
    self.checkWins =  function(player1_choice, player2_choice) {
      //  console.log("Entered the check wins");
      ...
      // R  P  S
      if(self.getChoiceAsNumber(a) == 0 ){   //  if p1 r
        if(self.getChoiceAsNumber(b) == 1 ){
          // console.log("P2 Won the round!");
          return 2;
        }else if(self.getChoiceAsNumber(b) == 0){
          return 0;
        }else{
          // console.log("P1 Won the round!");
          return 1;
        }
      }else if(self.getChoiceAsNumber(a) == 1 ){  // if p1 p
        if(self.getChoiceAsNumber(b) == 2 ){
          // console.log("P2 Won the round!");
          return 2;
        }else if(self.getChoiceAsNumber(b) == 1){
          return 0;
        }else{
          // console.log("P1 Won the round!");
          return 1;
        }
      }else if(self.getChoiceAsNumber(a) == 2 ){  // if p1 s
        if(self.getChoiceAsNumber(b) == 0 ){   // if p2 r
          // console.log("P2 Won the round!");
          return 2;
        }else if(self.getChoiceAsNumber(b) == 2){  // if p2 s
          return 0;
        }else{   // if p2 p
          // console.log("P1 Won the round!");
          return 1;
  ...
  $scope.onNext = function (choice) {
    ...
    // Additionally check if player 2 has best 2/3
    {
      ...
      if(window.plugins && window.plugins.nativepagetransitions) {
        ...
        $state.go('player2', {'player1_choice': choice,'player2_choice': 0,'player1_wins': $stateParams.player1_wins,'player2_wins': $stateParams.player2_wins});
      }
    }
  };
});
...
lab2.controller('player2', function(processMove, $rootScope, $scope, $ionicModal, $ionicLoading, $ionicViewSwitcher, $state, $stateParams, navigation) {
  ...
  $scope.onNext = function (choice) {
    ...
    var winner_var = $scope.processMove.checkWins($stateParams.player1_choice, choice);
    ...
    else if ($stateParams.player2_wins >= 2) // Check if player2 has best 2/3
    {
      ...
      if(window.plugins && window.plugins.nativepagetransitions) {
        ...
        $state.go('player1', {'player1_choice' : 0,'player2_choice' : 0,'player1_wins' : $stateParams.player1_wins,'player2_wins' : $stateParams.player2_wins,
        })
      }
    }
  };
